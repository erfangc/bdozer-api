...
class FilingProviderImpl(
    ...
    //
    private val instanceDocument =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$instanceDocumentFileName")?.inputStream() ?: error("..."))
    private val schemaExtension =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$schemaExtensionFilename")?.inputStream() ?: error("..."))
    private val labelLinkbase =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$labelFilename")?.inputStream() ?: error("..."))
    private val definitionLinkbase =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$definitionFileName")?.inputStream() ?: error("..."))
    private val presentationLinkbase =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$presentationFileName")?.inputStream() ?: error("..."))
    private val calculationLinkbase =
        XbrlUtils.readXml(http.readLink("${baseUrl}/$calculationFileName")?.inputStream() ?: error("..."))
